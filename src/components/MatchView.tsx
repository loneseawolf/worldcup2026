...
import type { Lang, Match, MatchSide, Official, TeamLineup } from '../types'
...
import { isDarkTheme, teamBarColors } from '../utils/teamAccent'
import '../pages/matchdetail.css'
...
// FMHY ("Free Media Heck Yeah") community-maintained free live-TV index
...
/**
 * Full match page body, shared by the canonical `/match/:id` route and the Live
 * page. Renders the hero (score + scorers + red cards), win probability,
 * commentary, the venue/weather/officials info grid, team stats, the lineups
 * pitch with substitutes, and head-to-head. When `live` is true the
 * "where to watch" info card is hidden and a prominent link back to the
 * canonical match page is shown.
 */
export default function MatchView({ m, live = false }: { m: Match; live?: boolean }) {
  ...
  // contrast- + distinctness-guarded team fills for the pitch dots/labels
  const barColors = teamBarColors(
    m.home ? teams[m.home.code]?.colors : undefined,
    m.away ? teams[m.away.code]?.colors : undefined,
    ...
    <div>
      ...
      <div className="card md-hero">
        <div className="md-hero-top">
          ...
          {live && (
            <Link className="md-fullmatch-link" to={`/match/${m.id}`}>
              {t('viewFullMatch')}
      ...
      <div className="md-grid">
        ...
        {/* third-party (unofficial) free live-stream index — FMHY */}
        <section className="card card-pad md-streams">
          <h3 className="md-info-title">
            <Icon name="broadcast" size={18} />
            {t('liveTvStreamsHead')}
            <span className="chip md-streams-tag">{t('liveTvUnofficial')}</span>
          </h3>
          <p className="muted small md-streams-body">{t('liveTvStreamsBody')}</p>
          <a
            className="btn btn-primary md-streams-cta"
            href={FMHY_URL}
            target="_blank"
            rel="noopener noreferrer"
          >
      ...
      {hasLineups && lu ? (
        <div className="card md-pitch-card">
          <Pitch
            ...
            ratings={ratings}
            homeColor={barColors.home}
            awayColor={barColors.away}
          />
